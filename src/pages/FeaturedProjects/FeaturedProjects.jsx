...
function SvgIconPin() {
  return (
    <svg
      ...
      className="inline-block mr-1.5 text-brand-gold shrink-0"
    >
      <path
...
function SvgActionArrow() {
  return (
    <svg
      ...
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className="transition-transform duration-300 group-hover:translate-x-1 shrink-0 text-brand-gold"
    >
      <circle
        cx="12"
        cy="12"
...
export default function FeaturedProjects() {
  return (
    <section
      className="relative w-full bg-brand-white border-b border-brand-darkblue/[0.06] text-brand-darkblue py-6 sm:py-10 lg:py-12 overflow-hidden font-body"
      id="featured-projects"
    >
      <div className="absolute top-[70px] right-[15%] w-10 h-10 rounded-full border border-brand-gold/30 pointer-events-none z-[1]" />
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-4">
            <h2 className="font-display text-3xl sm:text-4xl lg:text-5xl font-black tracking-tight text-brand-darkblue uppercase leading-tight m-0">
              Our Ongoing <span className="text-brand-gold">Projects.</span>
            </h2>
          </div>
        </motion.div>
          ...
          {ongoingProjects.map((project, index) => {
            return (
              <motion.div
                ...
                className="group w-full bg-white border border-brand-darkblue/[0.08] rounded-2xl p-5 sm:p-6 lg:p-7 shadow-[0_4px_24px_rgba(0,0,0,0.03)] hover:shadow-[0_16px_40px_rgba(212,175,55,0.12)] hover:border-brand-gold/60 transition-all duration-300 text-left"
              >
                <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-6 lg:gap-8">
                  <div className="w-full lg:w-[280px] xl:w-[320px] aspect-[16/10] shrink-0 rounded-xl overflow-hidden bg-brand-white shadow-sm relative">
                    <img
                      src={project.image}
                      alt={project.title}
                  ...
                  <div className="flex-1 flex flex-col justify-center gap-3">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-[11px] sm:text-xs font-bold text-brand-gold uppercase tracking-wider">
                        {project.client} — {project.clientFullName}
                      </span>
                    </div>

                    <h3 className="font-display text-xl sm:text-2xl lg:text-[24px] xl:text-[26px] font-black text-brand-darkblue uppercase leading-tight tracking-tight group-hover:text-brand-gold transition-colors m-0">
                      {project.title}
                    </h3>

                    <div className="flex flex-col gap-2 pt-1 border-t border-brand-darkblue/[0.06]">
                      <div className="flex items-start gap-1.5 text-xs sm:text-sm">
                        <SvgIconPin />
                        <span className="text-brand-darkblue/70 font-medium shrink-0">
                          Client &nbsp;:
                        </span>
                      ...
                      <div className="flex items-start gap-1.5 text-xs sm:text-sm">
                        ...
                        <span className="text-brand-darkblue/70 leading-relaxed">
                          {project.scope}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="shrink-0 flex lg:flex-col items-center justify-between lg:justify-center gap-3 border-t lg:border-t-0 lg:border-l border-brand-darkblue/[0.06] pt-4 lg:pt-0 lg:pl-8">
                    <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-brand-gold/10 border border-brand-gold/30">
                    ...
                    <div className="hidden lg:flex items-center justify-center w-10 h-10 rounded-full bg-brand-white border border-brand-darkblue/[0.08] group-hover:border-brand-gold group-hover:bg-brand-gold/10 transition-colors">
                      <SvgActionArrow />
                    </div>
                  </div>
                </div>
