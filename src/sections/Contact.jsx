...
const metaItems = [
  { label: 'Headquarters', value: 'Sheikh Zayed Road, Dubai, UAE' },
  { label: 'Email Enquiry', value: 'info@3circles.ae' },
  { label: 'Phone Call', value: '+971 4 333 3333' },
];
...
export default function Contact() {
  ...
  return (
    <section id="contact" className="relative py-14 md:py-16 bg-bg-secondary border-b border-border-color overflow-hidden">
      <div className="w-full max-w-[1440px] mx-auto px-6 md:px-12 lg:px-16">
        <div className="grid grid-cols-1 lg:grid-cols-[0.75fr_1.25fr] gap-16 lg:gap-24 items-start">

          <div>
            <FadeUpText>
              <span className="font-display text-[9px] font-extrabold tracking-[0.35em] text-accent-gold uppercase block mb-5">
                Get In Touch
              </span>
            </FadeUpText>
            <h2 className="font-display text-4xl md:text-5xl xl:text-[56px] font-extrabold leading-[1.05] tracking-tight text-text-primary uppercase mb-10">
              <WordReveal text="Let's Build" delay={0} />
              <br />
              <WordReveal text="What's Next." delay={0.05} />
            </h2>

            <FadeUpText delay={0.2}>
              <div className="flex flex-col gap-8">
                {metaItems.map((item) => (
                  <div key={item.label} className="flex flex-col gap-1 pb-8 border-b border-border-color last:border-b-0 last:pb-0">
                    <span className="font-display text-[9px] font-extrabold tracking-[0.2em] text-text-secondary uppercase">
                      {item.label}
                    </span>
